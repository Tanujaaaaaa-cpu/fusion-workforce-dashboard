    ...
    <div className="rounded-2xl bg-white/70 backdrop-blur border border-slate-200 shadow-sm p-4">
      ...
      {sub && (
        <div className="text-xs text-slate-500 mb-2 flex items-center gap-1">
          <Info className="w-3.5 h-3.5" /> {sub}
        </div>
      )}
      {children}
    </div>
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            } h-full w-[360px] bg-white z-50 shadow-xl border-slate-200 border`}
...
function Button({ children, onClick, variant = "default", icon: Icon, className = "" }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-xl text-sm flex items-center gap-2 ${styles[variant]} ${className}`}
    >
...
function MiniStat({ label, value, tone = "slate" }) {
  ...
  return (
    <div
      className={`rounded-xl px-2.5 py-2 text-[12px] border border-slate-200 ${tones[tone]}`}
    >
      ...
      <div className="max-h-[calc(100vh-120px)] overflow-auto pr-1 flex flex-col gap-3">
              ...
              return (
                ...
                >
                  <div className="flex gap-3 items-center">
                    <div
                      className={`h-10 w-10 rounded-xl flex items-center justify-center text-white ${meta.color}`}
                    >
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <MiniStat
              label="Outcome"
              value={`${snapshot.outcome} / 100`}
              tone="indigo"
            />
            <MiniStat
              label="Green share"
              value={`${snapshot.adoption}%`}
              tone="green"
            />
            <MiniStat
              label="Shadow AI"
              value={`${snapshot.shadow}%`}
              tone="amber"
            />
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button variant="ghost" icon={Printer} onClick={onPrint}>
              Print / Save PDF
            </Button>
          </div>
        </div>
...
export default function FusionFalseDashboard() {
  ...
  useEffect(() => {
    async function loadConfig() {
      try {
        ...
        // If your FastAPI wraps the payload, adjust here (e.g. data.config)
        setDbConfig(data);
      } catch (err) {
        console.error("Failed to load fusion dashboard config", err);
        setDbError(err.message || "Failed to load config");
      } finally {
        setDbLoading(false);
      }
    }
  ...
  const defaultFunctionShareConfig =
    dbConfig?.defaultFunctionShare ?? DEFAULT_FUNCTION_SHARE;
  ...
  const [adoptGreen, setAdoptGreen] = useState(
    commonConfig.adoptionGAR[0].value
  );
  const [adoptAmber, setAdoptAmber] = useState(
    commonConfig.adoptionGAR[1].value
  );
  ...
  const [functionShares, setFunctionShares] = useState(
    defaultFunctionShareConfig
  );
  ...
  const totals = useMemo(() => {
    const employees = functionsWithCounts.reduce(
      (a, r) => a + r.employees,
      0
    );
    const contractors = functionsWithCounts.reduce(
      (a, r) => a + r.contractors,
      0
    );
  ...
  const reactiveProactiveCounts = useMemo(() => {
    const [rxPct, pxPct] = useCustomMix
      ? [customReactive, 100 - customReactive]
      : lensData.reactiveProactivePct;
    return allocateByShare(
      totalHeadcount,
      [
        { name: "Reactive", pct: rxPct },
        { name: "Proactive", pct: pxPct },
      ],
      "pct",
      "count"
    );
  ...
  const onOffCounts = useMemo(() => {
    const [onPct, offPct] = useCustomMix
      ? [customOnsite, 100 - customOnsite]
      : lensData.onOffPct;
    return allocateByShare(
      totalHeadcount,
      [
        { name: "On-site", pct: onPct },
        { name: "Remote", pct: offPct },
      ],
      "pct",
      "count"
    );
  ...
  const FALLBACK_CITIES = [
    ...
    { name: "New York", coords: [-74.006, 40.7128], people: 11, remote: 46, team: "HR" },
  ];
  ...
  const narration = [
    ...
    `Estimated labour debt: ${lostDaysOrg.toFixed(0)} days/week (~${lostHoursOrg.toFixed(
      0
    )} hrs/week ⇒ ${currency}${(lostCostWeek / 1000).toFixed(1)}k/week).`,
  ];
    ...
    <div className="relative min-h-screen w-full bg-transparent text-slate-900 print:bg-white text-[15px] md:text-[16px]">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto px-4 pt-6 pb-2 print:px-0">
          ...
          <div className="mt-2 text-xs text-slate-600 flex items-center gap-4">
            <div>
              <strong>Total:</strong> {totalHeadcount}
            </div>
            <div>
              <strong>Employees:</strong> {totals.employees}
            </div>
            <div>
              <strong>Contractors:</strong> {totals.contractors}
            </div>
          ...
          <main className="flex-1">
                ...
                <div className="hidden md:flex items-center gap-2 text-xs">
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white border">
                    <span
                      ...
                      style={{ background: MAP_THEME.bubble.remote }}
                    />
                    More Remote
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white border">
                    <span
                      className="h-2.5 w-2.5 rounded-full"
                      style={{ background: MAP_THEME.bubble.mixed }}
                    />
                    Mixed
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white border">
                    <span
                      className="h-2.5 w-2.5 rounded-full"
                      style={{ background: MAP_THEME.bubble.onsite }}
                    />
                    More On-site
                  </span>
                </div>
            ...
            >
              <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-white">
                ...
                <div className="absolute right-3 top-3 z-10 flex flex-col gap-2">
                  <button
                    onClick={() =>
                      setMapZoom((z) => Math.min(8, +(z + 0.4).toFixed(2)))
                    }
                  ...
                  <button
                    onClick={() =>
                      setMapZoom((z) => Math.max(0.9, +(z - 0.4).toFixed(2)))
                    }
                    className="w-8 h-8 rounded-lg bg-white border shadow text-slate-700 hover:bg-slate-50"
                    title="Zoom out"
                  >
                  ...
                  >
                    ...
                    >
                      ...
                      {CITIES.map((c, i) => {
                        const size = Math.max(
                          6,
                          Math.min(28, Math.sqrt(c.people) * 3.5)
                        );
                        const color =
                          c.remote >= 60
                            ? MAP_THEME.bubble.remote
                            : c.remote <= 40
                            ? MAP_THEME.bubble.onsite
                          ...
                          <Marker
                            ...
                            onClick={() =>
                              openSiteFocus({
                                ...
                                active: c.people,
                                remote: Math.round(
                                  (c.people * c.remote) / 100
                                ),
                                onsite:
                                  c.people -
                                  Math.round((c.people * c.remote) / 100),
                                fn: { [c.team]: c.people },
                              })
                            }
                          >
                            ...
                            <text
                              textAnchor="middle"
                              y={-(size + 10)}
                              className="fill-slate-800 text-[8px] font-medium"
                            >
                              {c.name} · {c.people}
                            </text>
                          </Marker>
                      ...
                      >
                        <div className="rounded-xl bg-white/95 shadow-lg border px-3 py-2 text-xs">
                          ...
                          <div className="text-slate-600">
                            Active: {hoverCity.people}
                          </div>
                          <div className="text-slate-600">
                            Remote: {hoverCity.remote}%
                          </div>
                          <div className="text-slate-500">
                            Primary team: {hoverCity.team}
                          </div>
                          <div className="mt-1 text-[10px] text-slate-400">
                            Click bubble to open Site Focus
                          </div>
                        </div>
                      </motion.div>
                    )}
            ...
            <div className="mt-4 grid gap-4 justify-items-center [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
              {canShowCard(lens, "adoption") && (
                <Card
                  icon={Activity}
                  title={<span className="font-bold">Adoption status</span>}
                  value={`${adoptionPercent}% Green`}
                  hint="Consistent across lenses"
                >
              ...
              {canShowCard(lens, "shadow") && (
                <Card
                  icon={ShieldAlert}
                  title={<span className="font-bold">Shadow AI usage</span>}
                  value={`${lensData.shadowAI}%`}
                  hint="Unapproved AI tools (weekly)"
                >
                  <div className="mt-3">
                    <BarMini value={lensData.shadowAI} />
                  </div>
                  <div className="text-xs text-slate-500 mt-2">
                    Enable enterprise AI with guardrails; convert risk → value.
                  </div>
                </Card>
              )}
              ...
              {canShowCard(lens, "outcome") && (
                <Card
                  icon={LineChartIcon}
                  title={<span className="font-bold">Outcome index</span>}
                  value={`${outcomeIndex} / 100`}
                  hint="Avg of Outcome series"
                >
                  <div className="mt-3">
                    <Sparkline
                      data={lensData.outputVsOutcome.map((d) => d.outcome)}
                    />
                  </div>
                  <div className="text-xs text-slate-500 mt-2">
                    Raising proactive work + automation lifts the green curve
                    faster.
                  </div>
                </Card>
              )}
              ...
              {canShowCard(lens, "labour") && (
                <Card
                  ...
                  hint={`${lostHoursOrg.toFixed(0)} hrs/wk • ${currency}${(
                    lostCostWeek / 1000
                  ).toFixed(1)}k/wk`}
                >
                  <div className="mt-3 text-xs text-slate-500">
                    Paying {paidDays}d; receiving ~{prodDays.toFixed(1)}d →{" "}
                    {(paidDays - prodDays).toFixed(1)}d gap × {totalHeadcount}{" "}
                    ppl.
                  </div>
                </Card>
              )}
            ...
            <div className="mt-2 grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
              <SimpleBadge
                label="Device match"
                value={
                  commonConfig.deviceTrust.deviceMatch ? "Verified" : "Mismatch"
                }
                tone={
                  commonConfig.deviceTrust.deviceMatch ? "green" : "red"
                }
              />
              <SimpleBadge
                label="Geofence"
                value={commonConfig.deviceTrust.geofence}
                tone="indigo"
              />
              <SimpleBadge
                label="Trust score"
                ...
                tone={
                  commonConfig.deviceTrust.trustScore >= 80
                    ? "green"
                    : commonConfig.deviceTrust.trustScore >= 60
                    ? "amber"
                    : "red"
                }
              />
              <SimpleBadge
                label="Policy"
                value={
                  commonConfig.deviceTrust.policyCompliant
                    ? "Compliant"
                    : "Breach"
                }
                tone={
                  commonConfig.deviceTrust.policyCompliant ? "green" : "red"
                }
              />
            </div>
            ...
            <div
              ...
              style={{
                gridTemplateColumns:
                  "repeat(auto-fit, minmax(500px, 500px))",
              }}
            >
              {canShowSection(lens, "output_outcome") && (
                <Section
                  ...
                  icon={Workflow}
                  sub="Output = effort; Outcome = value. Closing the gap improves ROI."
                >
                  {viewTab === "Outcome" ? (
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart
                          data={lensData.outputVsOutcome}
                          margin={{
                            top: 10,
                            right: 10,
                            left: -20,
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <Card
                        title="Outcome Index"
                        value={`${outcomeIndex} / 100`}
                        hint="Avg of monthly outcomes"
                      />
                      <Card
                        title="GAR Green"
                        value={`${adoptGreen}%`}
                        hint="Consistent adoption value"
                      />
                      <Card
                        title="Proactive mix"
                        value={`${
                          useCustomMix
                            ? 100 - customReactive
                            : lensesConfig[lens].reactiveProactivePct[1]
                        }%`}
              ...
              {canShowSection(lens, "fusion") && (
                <Section
                  title="Human–Digital Fusion by Function"
                  icon={Cpu}
                  sub="Task mix (not headcount): % human vs digital by function. Click a bar to drill."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={lensData.fusion}
                        margin={{
                          top: 10,
                          right: 10,
                          left: -20,
                      ...
                      >
                        ...
                        <Bar
                          ...
                          fill={COLORS.slate}
                          radius={[6, 6, 0, 0]}
                          onClick={(d) => openFunctionFocus(d.team)}
                          cursor="pointer"
                        />
                        <Bar
                          dataKey="digital"
              ...
              {canShowSection(lens, "reactive_proactive") && (
                <Section
                  title="Reactive vs Proactive (counts)"
                  icon={Activity}
                  sub="Reactive = interrupts; Proactive = planned improvements. Aim to grow proactive."
                >
                  <div className="h-64 grid grid-cols-2 gap-3">
                    <div className="flex flex-col items-center justify-center">
                      <PieChart width={170} height={170}>
                        <Pie
                          data={reactiveProactiveCounts}
                          dataKey="count"
                          outerRadius={70}
                        >
                          {reactiveProactiveCounts.map((_, i) => (
                            <Cell
                              key={`rp-${i}`}
                              fill={i === 0 ? COLORS.rose : COLORS.green}
                            />
                          ))}
                        </Pie>
                      </PieChart>
                      <div className="text-sm flex items-center gap-6 flex-nowrap whitespace-nowrap overflow-visible w-full">
                        <Badge tone="red">
                          Reactive {reactiveProactiveCounts[0].count}
                        </Badge>
                        <span className="mx-2">→</span>
                        <Badge tone="green">
                          Proactive {reactiveProactiveCounts[1].count}
                        </Badge>
                      </div>
                    </div>
                    <div className="text-xs text-slate-600 flex items-center">
                      Shift 15–20% of reactive to proactive pods; automate L1
                      triage to improve outcome & cut cost.
                    </div>
                  </div>
                </Section>
              )}
              ...
              {canShowSection(lens, "onoff") && (
                <Section
                  title="On-site vs Remote (counts)"
                  icon={Globe2}
                  sub="Where work is performed. Useful for cost, compliance, and scheduling."
                >
                  <div className="h-64 grid grid-cols-2 gap-3">
                    <div className="flex flex-col items-center justify-center">
                      <PieChart width={170} height={170}>
                        ...
                        >
                          {onOffCounts.map((_, i) => (
                            <Cell
                              key={`ol-${i}`}
                              fill={i === 0 ? COLORS.indigo : COLORS.zinc}
                            />
                          ))}
                        </Pie>
                      </PieChart>
                      <div className="text-sm flex items-center gap-6 flex-nowrap whitespace-nowrap overflow-visible w-full">
                        <Badge tone="indigo">
                          On-site {onOffCounts[0].count}
                        </Badge>
                        <span className="mx-2">/</span>
                        <Badge>
                          Remote {onOffCounts[1].count}
                        </Badge>
                      </div>
                    </div>
                    <div className="text-xs text-slate-600 flex items-center">
                      Move run-work to best-shore remote (policy permitting);
                      keep outcome-critical roles near users.
                    </div>
                  </div>
                </Section>
              )}
              ...
              {canShowSection(lens, "emp_contractor_org") && (
                <Section
                  title="Employee vs Contractor (Org)"
                  icon={Users}
                  sub="Employees anchor IP & continuity; contractors flex for peaks & run."
                >
                  <div className="h-64 grid grid-cols-2 gap-3">
                    <div className="flex flex-col items-center justify-center">
                      <PieChart width={170} height={170}>
                    ...
                    <div className="text-xs text-slate-600 flex items-center">
                      Current mix:{" "}
                      {Math.round((totals.employees / totalHeadcount) * 100)}%
                      employees /{" "}
                      {Math.round((totals.contractors / totalHeadcount) * 100)}%
                      contractors.
                    </div>
                  </div>
                </Section>
              ...
              {canShowSection(lens, "emp_contractor_fn") && (
                <Section
                  title="Employee vs Contractor by Function"
                  icon={Users}
                  sub="Click a bar to focus a function."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
              ...
              {canShowSection(lens, "holiday_sickness") && (
                <Section
                  title="Holiday & Sickness (trend)"
                  icon={Users}
                  sub="Align PTO & on-call with release windows to stabilise delivery."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={commonConfig.holidaySickness}
                        margin={{
                          top: 10,
                          right: 10,
                          left: -20,
              ...
              {canShowSection(lens, "license_util") && (
                <Section
                  title="License Utilisation (GAR)"
                  icon={Building2}
                  sub="Quick view of value vs waste across major platforms."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={lensData.licenseUtil}
                        margin={{
                          top: 10,
                          right: 10,
                          left: -20,
              ...
              {canShowSection(lens, "liability_asset") && (
                <Section
                  title="Liability vs Asset Curve (illustrative)"
                  icon={FileWarning}
                  sub="Aim to move points up/right (asset) and reduce left/down (liability)."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <ScatterChart
              ...
              {canShowSection(lens, "region_cost") && (
                <Section
                  title="Region Cost (to London baseline)"
                  icon={Factory}
                  sub="Indicative labour index (London=100)."
                >
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={regionCostConfig}
                        margin={{
                          top: 10,
                          right: 10,
                          left: -20,
              ...
              {canShowSection(lens, "anomalies") && (
                <Section
                  title="Anomalies & Next Actions"
                  icon={ShieldAlert}
                  sub="Curated signals with recommended next steps."
                >
            ...
            <div className="py-6 text-xs text-slate-500 print:hidden">
              This is an interactive, illustrative dashboard for Human + Digital
              fusion, dynamic pricing, and workforce optimisation.
            </div>
          </main>
        </div>
      </div>
      ...
      >
        <div className="text-xs text-slate-600 mb-3">
          Tune assumptions to see the story update live.
        </div>
        <SliderRow
          ...
          onChange={setTotalHeadcount}
          suffix=""
          hint="Rescales all function counts."
        />

        <div className="mt-4 font-semibold text-slate-700 mb-1">
          Adoption (constant across lenses)
        </div>
        <SliderRow
          label="Green"
          value={adoptGreen}
          onChange={setAdoptGreen}
        />
        <SliderRow
          label="Amber"
          value={adoptAmber}
          onChange={setAdoptAmber}
        />
        ...
        <SwitchRow
          label="Use custom Reactive/Proactive & On-site/Remote"
          checked={useCustomMix}
          onChange={setUseCustomMix}
        />
        {useCustomMix && (
          <>
            <SliderRow
              label="Reactive %"
              value={customReactive}
              onChange={setCustomReactive}
            />
            <div className="text-[11px] text-slate-500 mb-2">
              Proactive auto = {100 - customReactive}%
            </div>
            <SliderRow
              label="On-site %"
              value={customOnsite}
              onChange={setCustomOnsite}
            />
            <div className="text-[11px] text-slate-500 mb-2">
              Remote auto = {100 - customOnsite}%
            </div>
          </>
        )}
        ...
        <SliderRow
          label="Productive days / week"
          min={3.0}
          max={4.8}
          step={0.1}
          value={prodDays}
          onChange={setProdDays}
          suffix="d"
          hint="Raises or lowers labour debt."
        />
        ...
        <div className="text-[11px] text-slate-500 mb-4">
          Lost ≈ {lostHoursOrg.toFixed(0)} hrs/week ⇒ {currency}
          {(lostCostWeek / 1000).toFixed(1)}k/week
        </div>

        <div className="mt-4 font-semibold text-slate-700 mb-1">
          Function Shares (must sum ≈ 100%)
        </div>
        {functionShares.map((f, idx) => (
          <SliderRow
            ...
            value={f.sharePct}
            onChange={(v) =>
              updateShare(idx, v, functionShares, setFunctionShares)
            }
          />
        ))}
        <div className="text-[11px] text-slate-500 mb-3">
        ...
        {Object.keys(empPctByFn).map((team) => (
          <SliderRow
            ...
            value={empPctByFn[team]}
            onChange={(v) =>
              setEmpPctByFn((prev) => ({ ...prev, [team]: v }))
            }
          />
        ))}
      ...
      >
        <LensSidebar
          ...
          snapshot={{
            ...
            contractors: totals.contractors,
            outcome: Math.round(
              avg(lensesConfig[lens].outputVsOutcome.map((d) => d.outcome))
            ),
      ...
      <Drawer
        ...
        title={
          selectedSite
            ? `Site Focus: ${selectedSite.city}`
            : `Function Focus: ${selectedFunction ?? ""}`
        }
        side="left"
      >
        {selectedSite ? (
          ...
          <div className="text-sm text-slate-600">
            Click a city bubble on the map, or a bar in “Fusion by Function”.
          </div>
        )}
      </Drawer>
    </div>
...
function FunctionDetails({
  ...
  const estAutomationSavings =
    estWeeklyHours * estDigitalShare * 0.15 * estHourly;
  ...
  return (
    <div className="text-sm">
      <div className="mb-2">
        ...
        <div className="text-slate-500">
          Headcount: {f.total} • Employees: {f.employees} • Contractors:{" "}
          {f.contractors}
        </div>
      </div>
      ...
      <div className="rounded-xl border p-3 mb-3">
        <div className="text-xs text-slate-500 mb-1">
          What-If Savings (illustrative)
        </div>
        <div className="text-slate-700">
          Weekly Hours ≈ {estWeeklyHours.toLocaleString()} • Digital share{" "}
          {Math.round(estDigitalShare * 100)}%
          <br />
          Potential extra leverage (15% of digital hours):{" "}
          <strong>
            {currency}
            {(estAutomationSavings / 1000).toFixed(1)}k / week
          </strong>
        </div>
        <div className="text-[11px] text-slate-500 mt-1">
          Uses a simple assumption to make the conversation tangible.
        </div>
      </div>
...
function SiteDetails({ site, currency, onClose }) {
  if (!site) return null;
  const remoteShare = site.active
    ? Math.round((site.remote / site.active) * 100)
    : 0;
  ...
  return (
    <div className="text-sm">
      ...
      <div className="text-[11px] text-slate-500">
        This view is aggregated per city/site for privacy. Wire this to your
        FastAPI endpoint that returns site presence (active/onsite/remote) for
        real data.
      </div>
...
function normalizeShares(shares) {
  ...
  return shares.map((s) => ({
    ...s,
    sharePct: Math.max(0, Math.round((s.sharePct / sum) * 100)),
  }));
}
function updateShare(idx, value, shares, setShares) {
  const next = shares.map((s, i) =>
    i === idx ? { ...s, sharePct: value } : s
  );
  setShares(next);
}
